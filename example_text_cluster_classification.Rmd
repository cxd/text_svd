...
- $U$ will be a matrix of the dimension $m \times k$ representing the eigenvectors for the rows of the matrix $A$

- $S$ will be a vector of dimension $k$ representing the eigenvalues for matrix $A$.

- $V'$ is the matrix of dimension $k \times n$ representing the eivenvectors of the columns of the matrix $A$.
...
A scree plot can be used to visualise the contribution of the components.
...
Hence we can investigate the use of the library "scatterD3" to support the visualisation.
...
scatterD3(plotData, x=y, y=y2, lab=labels, point_opacity=0.2, col_var=cluster, legend_width=0,
          size_var=size)
...
The resulting dendogram is much harder to separate, we can attempt to use the intuition from the previous clustering in order to choose approximately 20 clusters and plot the search space.

```{r}
cuts2 <- cutree(hc2, k=20)
hist(cuts2, breaks=20)
plotData <- data.frame(x=Vs[,2], y=Vs[,3],labels=unique_words[1:length(Vs[,2])], cluster=cuts2[1:length(Vs[,2])], size=term_freq$count[1:length(Vs[,2])])
scatterD3(plotData, x=x, y=y, lab=labels, point_opacity=0.2, col_var=cluster, legend_width=0
          ,size_var=size)
```

From the histogram of the cuts we can see this method does not produce good separation of clusters at all. We can instead investigate the kmeans method of clustering.

```{r}
k <- kmeans(Vs, 20)
plotData <- data.frame(x=Vs[,1], y=Vs[,2],labels=unique_words[1:length(Vs[,2])], cluster=k$cluster, size=term_freq$count[1:length(Vs[,2])])
...
```
The above plot suggests that there may be some separability based on clustering the decomposition of documents. Applying the same data but using the app_tag as a label produces a different representation.
