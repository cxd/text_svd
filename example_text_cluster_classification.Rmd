...
Note that there is some question as to the suitability of the hand labelled tags sem_tag and app_tag, do they segment the utterances effectively? The process of hand labelling within the telephony domain is largely subjective and is determined by the consultation process between an analyst and an end customer. It is also hampered to some extent due to the process necessarily occuring on a subset of examples.
...
The first 100 components are plotted as follows.

```{r}

plot(eigenV[1:100], type="b")

plot(eigenV[1:100]/total, type="b")
```
...
Note the axes on the first dimension is relatively close, rotating about the other axis provides additional view of the data.

We can explore the space of one projection to examine the terms that are located in the ordination. Given that the area between -0.1 and 0.1 is quite dense it is possible to explore this further. However interactive graphics of course would allow much more detailed exploration of the terms in the vector space.
...
scatterplot3js(x,y,z, pch=terms, size=0.5, font.symbols="size:10pt;", axis=TRUE, use.orbitcontrols=TRUE)
```
...
```{r, fig.width=20, fig.height=10}
D <- t(apply(Vs, 1, function(x)(x-min(x))/(max(x)-min(x))))
Ds <- as.dist(D)
...
```{r}
k <- kmeans(Vs, 20)
plotData <- data.frame(x=Vs[,20], y=Vs[,30],labels=unique_words[1:length(Vs[,2])], cluster=k$cluster, size=term_freq$count[1:length(Vs[,2])])
...
We can also work with the projection onto the set of key components by projecting the term matrix into the top $n$ components and generate the corresponding distance matrix. For our case we choose the top 100 components.
...
dim(Z)
M <- Z[,1:100]
# calculate distance matrix with each row being a term, and columns being the first 100 projections 
# of the covariance matrix
D <- dist(M)
hc3 <- hclust(D, method="centroid")
plot(hc3, hang=-1, cex=0.3)

```

The dimensionality is still quite difficult to visualise, however we will again make an estimate of 20 clusters, for demonstration purposes only.

```{r, webgl=TRUE}
cuts3 <- cutree(hc3, k=20)
hist(cuts3, breaks=20)
plotData <- data.frame(x=Z[,10], y=Z[,20],labels=unique_words[1:length(Z[,2])], cluster=cuts3[1:length(Z[,2])], size=term_freq$count[1:length(Z[,2])])
#scatterD3(plotData, x=x, y=y, lab=labels, point_opacity=0.2, col_var=cluster, legend_width=0
#          ,size_var=size)
...
y <- Z[,20]
z <- Z[,30]
terms <- unique_words[1:length(Z[,2])]
# colors for each cluster
col <- rainbow(20)
cols <- sapply(plotData$cluster, function (c) col[c])
scatterplot3js(x,y,z, pch=terms, size=0.5, font.symbols="size:10pt;", axis=TRUE, color=cols, use.orbitcontrols=TRUE)
